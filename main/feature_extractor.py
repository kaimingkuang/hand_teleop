...
def generate_features(visual_baked, model, preprocess, backbone_type="ResNet34", stack_frames=False, encode=False, num_data_aug=5, augmenter = T.AugMix()):
  ...
  if encode:
    ...
    with torch.no_grad():
      features = image_encoder.encode(features)
    features = features.cpu().detach().numpy()
    state_decoder = train_state_decoder(robot_states=robot_states)
    robot_states = torch.from_numpy(np.array(robot_states, dtype=np.float32))
    robot_states = robot_states.to(device)
    with torch.no_grad():
      robot_states = state_decoder.decode(robot_states)
    robot_states = robot_states.cpu().detach().numpy()
